...
public class DistributedTxParticipant extends DistributedTx implements Watcher {
    ...
    public void voteCommit() {
        try {
            if (currentTransaction != null) {
                ...
                client.write(currentTransaction, VOTE_COMMIT.getBytes(StandardCharsets.UTF_8));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void voteAbort() {
        try {
            if (currentTransaction != null) {
                ...
                client.write(currentTransaction, VOTE_ABORT.getBytes(StandardCharsets.UTF_8));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void handleRootDataChange() {
        try {
            ...
            if (GLOBAL_COMMIT.equals(dataString)) {
                listener.onGlobalCommit();
            } else if (GLOBAL_ABORT.equals(dataString)) {
